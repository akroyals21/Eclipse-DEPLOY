...
setup_wallet() {
    ...
    show "Do you want to import a private key (byte array) or create a new wallet?"
    PS3="Please enter your choice (1 or 2): "
    options=("Import private key (byte array)" "Create new wallet")
    select opt in "${options[@]}"; do
        case $opt in
            "Import private key (byte array)")
                show "Importing private key..."
                read -p "Enter your private key in byte array form (e.g., [1,2,3,...]): " BYTE_ARRAY_PRIVATE_KEY
                # Save the byte array to a JSON file
                KEYPAIR_PATH="$KEYPAIR_DIR/imported-keypair.json"
                echo "$BYTE_ARRAY_PRIVATE_KEY" > "$KEYPAIR_PATH"
                show "Private key imported and saved to $KEYPAIR_PATH."
                ;;
            "Create new wallet")
